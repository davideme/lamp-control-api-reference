...
| ADR                                                                      | Title                          | Decision                                                                                                  |
| ------------------------------------------------------------------------ | ------------------------------ | --------------------------------------------------------------------------------------------------------- |
| [ADR-007](src/typescript/docs/adr/007-postgresql-storage-with-prisma.md) | PostgreSQL Storage with Prisma | Use Prisma 5.x as the ORM with auto-generated TypeScript types; run Prisma Migrate for schema management. |

#### Python

| ADR                                                                      | Title                                  | Decision                                                                        |
| ------------------------------------------------------------------------ | -------------------------------------- | ------------------------------------------------------------------------------- |
| [ADR-007](src/python/docs/adr/007-postgresql-storage-with-sqlalchemy.md) | PostgreSQL Storage with SQLAlchemy 2.0 | Use SQLAlchemy 2.0 async engine with asyncpg driver and Alembic for migrations. |

#### Java

| ADR                                                                    | Title                                   | Decision                                                                                                                |
| ---------------------------------------------------------------------- | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| [ADR-007](src/java/adr/007-postgresql-storage-with-spring-data-jpa.md) | PostgreSQL Storage with Spring Data JPA | Use Spring Data JPA with Hibernate and HikariCP connection pool; Flyway manages schema migrations.                      |
| [ADR-008](src/java/adr/008-java-test-tools.md)                         | Java Test Tools Selection               | Use JUnit 5, Mockito, Testcontainers, AssertJ, Awaitility, and JaCoCo for comprehensive testing and coverage reporting. |

#### Kotlin

| ADR                                                              | Title                               | Decision                                                                      |
| ---------------------------------------------------------------- | ----------------------------------- | ----------------------------------------------------------------------------- |
| [ADR-006](src/kotlin/adr/006-postgresql-storage-with-exposed.md) | PostgreSQL Storage with Exposed ORM | Use JetBrains Exposed ORM with HikariCP for idiomatic Kotlin database access. |

#### Go

| ADR                                                      | Title                          | Decision                                                                                   |
| -------------------------------------------------------- | ------------------------------ | ------------------------------------------------------------------------------------------ |
| [ADR-005](src/go/adr/005-postgresql-storage-with-pgx.md) | PostgreSQL Storage with pgx v5 | Use pgx v5 with pgxpool for connection pooling and sqlc for type-safe SQL code generation. |

#### C#

| ADR                                                              | Title                           | Decision                                                                                           |
| ---------------------------------------------------------------- | ------------------------------- | -------------------------------------------------------------------------------------------------- |
| [ADR-007](src/csharp/adr/007-postgresql-storage-with-ef-core.md) | PostgreSQL Storage with EF Core | Use Entity Framework Core 8.0+ with the Npgsql provider; EF Core Migrations manage schema changes. |
...
| [ADR-005](src/java/adr/005-asynchronous-request-handling.md) | Async Strategy           | Spring MVC with `@Async` and `CompletableFuture` for I/O operations (not reactive WebFlux).                 |
...
| [ADR-007](src/java/adr/007-java-linter-formatter.md)         | Linter & Formatter       | Spotless + Google Java Format for formatting; SpotBugs, PMD, Checkstyle for static analysis.                |
...
| [ADR-003](src/kotlin/adr/003-gradle-build-tool.md) | Build Tool         | Gradle with Kotlin DSL — idiomatic, flexible, strong Kotlin/JVM ecosystem support. |
| [ADR-004](src/kotlin/adr/004-web-framework.md)     | HTTP Framework     | Ktor — JetBrains-maintained, coroutine-native, lightweight.                        |
| [ADR-005](src/kotlin/adr/005-openapi-generator.md) | OpenAPI Generation | OpenAPI Generator with `kotlin-server` template for server stub generation.        |
...
| [ADR-003](src/go/adr/003-go-modules.md)               | Dependency Management    | Go modules (`go.mod`) — standard Go toolchain, reproducible builds.              |
| [ADR-004](src/go/adr/004-oapi-codegen.md)             | OpenAPI Code Generation  | `oapi-codegen` — generates idiomatic Go server interfaces from the OpenAPI spec. |
...
| [ADR-003](src/csharp/adr/003-dotnet-build-tool.md)        | Build Tool               | `dotnet` CLI — standard .NET toolchain, excellent CI/CD integration.             |
...
[Unreleased]: https://github.com/your-org/lamp-control-api-reference/compare/v1.1.0...HEAD
[v1.1.0]: https://github.com/your-org/lamp-control-api-reference/compare/v1.0.0...v1.1.0
[v1.0.0]: https://github.com/your-org/lamp-control-api-reference/releases/tag/v1.0.0
