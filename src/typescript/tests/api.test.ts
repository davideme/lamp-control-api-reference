...
import { buildApp } from '../src/infrastructure/app.ts';
...
describe('Lamp API Endpoints', () => {
  ...
  describe('GET /v1/lamps/:lampId', () => {
    it('should return a specific lamp', async () => {
      ...
      const createResponse = await request.post('/v1/lamps').send({ status: true }).expect(201);
    ...
    it('should return 404 for non-existent lamp', async () => {
      await request.get('/v1/lamps/non-existent-id').expect(404);
    });
  });
  ...
  describe('POST /v1/lamps', () => {
    ...
    it('should validate request body', async () => {
      await request.post('/v1/lamps').send({ invalid: 'data' }).expect(400);
    });
  });
  ...
  describe('PUT /v1/lamps/:lampId', () => {
    ...
    it('should return 404 for non-existent lamp', async () => {
      await request.put('/v1/lamps/non-existent-id').send({ status: false }).expect(404);
    });
    ...
    it('should validate request body', async () => {
      ...
      await request.put(`/v1/lamps/${lampId}`).send({ invalid: 'data' }).expect(400);
    });
  });
  ...
  describe('DELETE /v1/lamps/:lampId', () => {
    it('should delete an existing lamp', async () => {
      ...
      await request.delete(`/v1/lamps/${lampId}`).expect(204);

      // Verify the lamp is deleted
      await request.get(`/v1/lamps/${lampId}`).expect(404);
    });

    it('should return 404 for non-existent lamp', async () => {
      await request.delete('/v1/lamps/non-existent-id').expect(404);
    });
  });
});
